...
export function compactForResize(
  layout: Layout,
  i: string,
  w: number,
  h: number,
...
/**
 * Determine if a is layout item is visible within parent for lazyloading 
 * 
 * @param {Object} l 
 * @param {Number} rowHeight 
 * @param {Number} parentContainerScrollTop 
 * @param {Number} parentContainerHeight 
 * @returns {Boolean}
 */
export function isLayoutItemVisible(
  l: LayoutItem,
  rowHeight: number,
  margin: [number, number],
  parentContainerScrollTop: number, 
  parentContainerHeight: number,
): boolean {

  //Get item top and bottom position (pixels) based on layout properties
  const top = (rowHeight * l.y) + margin[1];
  const bottom = top + margin[1] + (rowHeight * l.h);
  const scrollTop = parentContainerScrollTop;
  const scrollBottom = scrollTop + parentContainerHeight;

  //Determine visibility of the item based on the scroll position of the parent container
  const isTopVisible = (top >= scrollTop && top <= scrollBottom);
  const isBottomVisible = (bottom >= scrollTop && bottom <= scrollBottom);

  // Check if the element's height is greater than the viewport height
  const elementHeight = bottom - top;
  const viewportHeight = scrollBottom - scrollTop;
  const isElementTallerThanViewport = elementHeight > viewportHeight;

  // If the element is taller than the viewport, it's visible if the entire viewport lies within the element
  const isViewportWithinElement = isElementTallerThanViewport && (top <= scrollTop && bottom >= scrollBottom);

  return isTopVisible || isBottomVisible || isViewportWithinElement;
